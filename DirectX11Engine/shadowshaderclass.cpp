...
bool ShadowShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../DirectX11Engine/shadow.vs", L"../DirectX11Engine/shadow.ps");
	if (!result)
	{
		return false;
	}
...
bool ShadowShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			OutputShaderErrorMessage(errorMessage, hwnd, "VS shadow");
		}
		// If there was nothing in the error message then it simply could not find the shader file itself.
		else
		{
			MessageBox(hwnd, "vs shadow", "Missing Shader File", MB_OK);
		}
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			OutputShaderErrorMessage(errorMessage, hwnd, "ps shadow");
		}
		// If there was nothing in the error message then it simply could not find the file itself.
		else
		{
			MessageBox(hwnd, "ps shadow", "Missing Shader File", MB_OK);
		}
